			      ...
			      abb_comparador comparador)
{
	...
	if (cmp < 0) {
		nodo->derecha =
			abb_insertar_nodo(nodo->derecha, elemento, comparador);
	} else {
		nodo->izquierda = abb_insertar_nodo(nodo->izquierda, elemento,
						    comparador);
	}
	return nodo;
}
...
void swap_contenido_de_nodos_abb(nodo_abb_t *nodo_original,
				 nodo_abb_t *nodo_nuevo)
{
...
void *abb_quitar_nodo(nodo_abb_t *nodo, nodo_abb_t *padre, abb_t *arbol)
{
	...
	if (nodo->izquierda) {
		nodo_abb_t *padre_predecesor =
			buscar_padre_predecesor_inorden(nodo->izquierda);
		if (padre_predecesor == nodo->izquierda &&
		    !padre_predecesor->derecha) {
...
nodo_abb_t *abb_padre_de_nodo_buscado(nodo_abb_t *nodo, nodo_abb_t *padre,
				      void *elemento, abb_comparador comparador)
{
	...
	if (cmp < 0) {
		return abb_padre_de_nodo_buscado(nodo->derecha, nodo, elemento,
						 comparador);
	}
	return abb_padre_de_nodo_buscado(nodo->izquierda, nodo, elemento,
					 comparador);
}
...
void *abb_quitar(abb_t *arbol, void *elemento)
{
	...
	nodo_abb_t *nodo_padre =
		abb_padre_de_nodo_buscado(arbol->nodo_raiz, arbol->nodo_raiz,
	...
	if (nodo_padre == arbol->nodo_raiz &&
	    arbol->comparador(nodo_padre->elemento, elemento) == 0) {
		nodo_buscado = nodo_padre;
	} else if (nodo_padre->derecha != NULL &&
		   arbol->comparador(nodo_padre->derecha->elemento, elemento) ==
			   0) {
...
void *abb_buscar(abb_t *arbol, void *elemento)
{
	...
	if (nodo_padre == NULL)
		return NULL;
	else if (nodo_padre == arbol->nodo_raiz &&
		 arbol->comparador(nodo_padre->elemento, elemento) == 0) {
		return nodo_padre->elemento;
	} else if (nodo_padre->derecha &&
		   arbol->comparador(nodo_padre->derecha->elemento, elemento) ==
			   0) {
...
size_t abb_preorden(nodo_abb_t *nodo, bool (*funcion)(void *, void *),
		    void *aux, bool *ejecucion)
{
	if (!nodo || !(*ejecucion)) {
		return 0;
	}
...
size_t abb_postorden(nodo_abb_t *nodo, bool (*funcion)(void *, void *),
		     void *aux, bool *ejecucion)
{
	if (!nodo) {
		return 0;
			     ...
			     bool (*funcion)(void *, void *), void *aux)
{
	...
	case INORDEN:
		cuantos =
			abb_inorden(arbol->nodo_raiz, funcion, aux, &ejecucion);
		break;
	case POSTORDEN:
				  ...
				  bool *ejecucion)
{
	...
	(*p_contador) = abb_recorrer_nodo_preorden(
		nodo->izquierda, array, tamanio_array, p_contador, ejecucion);
	(*p_contador) = abb_recorrer_nodo_preorden(
		nodo->derecha, array, tamanio_array, p_contador, ejecucion);
	return *p_contador;
}
				 ...
				 bool *ejecucion)
{
	...
	(*p_contador) = abb_recorrer_nodo_inorden(
		nodo->izquierda, array, tamanio_array, p_contador, ejecucion);
	array[*p_contador] = nodo->elemento;
	(*p_contador)++;
	// imprime_array(array,(*p_contador));
				   ...
				   bool *ejecucion)
{
	if (!nodo)
		return (*p_contador);
	(*p_contador) = abb_recorrer_nodo_postorden(
		nodo->izquierda, array, tamanio_array, p_contador, ejecucion);
	(*p_contador) = abb_recorrer_nodo_postorden(
		nodo->derecha, array, tamanio_array, p_contador, ejecucion);
	if (*ejecucion) {
		    ...
		    size_t tamanio_array)
{
	...
	switch (recorrido) {
	case PREORDEN:
		contador = abb_recorrer_nodo_preorden(arbol->nodo_raiz, array,
						      tamanio_array, &contador,
						      &ejecucion);
		break;
	case INORDEN:
		contador = abb_recorrer_nodo_inorden(arbol->nodo_raiz, array,
						     tamanio_array, &contador,
						     &ejecucion);
		break;
	case POSTORDEN:
		contador = abb_recorrer_nodo_postorden(arbol->nodo_raiz, array,
						       tamanio_array, &contador,
						       &ejecucion);
		break;
	default:
